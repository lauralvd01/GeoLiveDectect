...
namespace MOT.CORE.Matchers.Deep
{
    public class DeepSortMatcher : Matcher
    {
        ...
        public override IReadOnlyList<ITrack> Run(Bitmap frame, float targetConfidence, params DetectionObjectType[] detectionObjectTypes)
        {
            ...
            /*
            List<IPrediction> filteredDetectedObj = new List<IPrediction>();
            for (int i = 0; i < detectedObjects.Count(); i++)
            {
                IPrediction obj = detectedObjects[i];
                if ((obj.CurrentBoundingBox.Height>0) && (obj.CurrentBoundingBox.Width > 0))
                    filteredDetectedObj.Add(obj);
            }
        ...
        //public override IPrediction[] Run_T_Predict(Bitmap frame, float targetConfidence, params DetectionObjectType[] detectionObjectTypes)          //todo rajouter (de meme pour Run_T_Match) les decalrations de fonction dans les autres classes (IMatcher et dans les autres matchers si l'on veut les utiliser en Thrad)
        public override IPrediction[] Run_T_Predict(Bitmap frame, float targetConfidence, params DetectionObjectType[] detectionObjectTypes)        // Run() versions pour Threads
        {
            IPrediction[] detectedObjects = _predictor.Predict(frame, targetConfidence, detectionObjectTypes).ToArray();
