...
{
	console.log("process_RenderMessage");
	...
	else 
	{
		...
		let msg = event.data;
		if(msg.startsWith("<Params><Message id=\""))
		{
			let pos = msg.indexOf("\"", 21);
			let msgId = Number(msg.substring(21, pos));
			
			pos = msg.indexOf("type=\"", pos);
			let pos2 = msg.indexOf("\"", pos+6);
			let typeMsg = msg.substring(pos+6, pos2);

			pos = msg.indexOf(">", pos2);
			pos2 = msg.indexOf("</Message>", pos);
			let message = msg.substring(pos+1, pos2);
			
			if (msgId == JSON_GR_SETTRACKERS)
			{			
				processGeoLiveDetect_rects(JSON.parse(message));
				return;
			}
		}


		msg = event.data.replace('type="txt" >', 'type="txt">');
...
function processGeoLiveDetect_rects(json)
{
	/*
	json = { "utcTime": timestamp, "tracks": 
	[
		{ "id": id, "bb": { "l": left, "r": right, "t": top, "b": bottom } },			//t (top) is < b (bottom) because it's start at top left 
		{ "id": id, "del": true }
	]};
	*/
	...
	let div = $(".preview .rects_container");
	div.empty();

	let rects = json.tracks;
	if((!ckv(json))||(rects.length==0))
		return;
	...
	// init les actions sur click => renvois un message avec {selected: [id0, id1, id3, ...], unselected: [id2, ...]
	let context = this;
	div.find(".rect").click(function()
	{
		$(this).toggleClass("selected");
		context.sendSelectedRectsToGeoDetectLive();
	});
