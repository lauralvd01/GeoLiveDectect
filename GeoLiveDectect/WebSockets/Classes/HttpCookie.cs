...
namespace System.Web
{
    ...
    public sealed class HttpCookie {
        ...
        private void SetDefaultsFromConfig() {
            //HttpCookiesSection config = RuntimeConfig.GetConfig().HttpCookies;                //todo voir si les config venant du RunTimeConfig (qui eux meme proviennes de plusieurs settings possibles) sont necessaire pour ici (apparement on n'utilise pas vraiment les cookies dans le WebSocketSErver ou Client)
            HttpCookiesSection config = new HttpCookiesSection();
        ...
        */
        private bool SupportsHttpOnly() { return true; }
        ...
        public static bool TryParse(string input, out HttpCookie result) {
            ...
            HttpCookie cookie = CreateCookieFromString(cookiePair.Trim());
        ...
        internal static HttpCookie CreateCookieFromString(String s)         //from HttpRequest.cs
        {
        ...
        internal HttpResponseHeader GetSetCookieHeader()
        {
            ...
            if (_expirationSet && _expires != DateTime.MinValue) {
                ...
                s.Append(FormatHttpCookieDateTime(_expires));
            }
            ...
            if (_sameSite > (false ? SameSiteMode.None : (SameSiteMode)(-1) /* Unspecified *//*))
            {
                s.Append("; SameSite=");
                s.Append(_sameSite);
            }
            ...
            return new HttpResponseHeader(27, s.ToString());
        }
        */
