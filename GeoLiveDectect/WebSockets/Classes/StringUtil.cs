...
internal static class StringUtil {
    ...
    internal static string CheckAndTrimString(string paramValue, string paramName,
        ...
        if (trimmedValue.Length == 0) {
            throw new ArgumentException("Input parameter '"+ paramName + "' cannot be an empty string.");
        }
        if (lengthToCheck > -1 && trimmedValue.Length > lengthToCheck) {
            throw new ArgumentException("Trimmed string value '"+ paramValue + "' of input parameter '"+ paramName + "' cannot exceed character length "+ lengthToCheck.ToString(CultureInfo.InvariantCulture) + ".");
        }
        return trimmedValue;
    }
    ...
    unsafe internal static bool Equals(string s1, int offset1, string s2, int offset2, int length) {
        ...
        if ((s1 == null ? 0 : s1.Length) - offset1 < length)
            throw new ArgumentOutOfRangeException("The sum of offset1 and length is greater than the length of the buffer.");
        if ((s2 == null ? 0 : s2.Length) - offset2 < length)
            throw new ArgumentOutOfRangeException("The sum of offset2 and length is greater than the length of the buffer.");
        if (String.IsNullOrEmpty(s1) || String.IsNullOrEmpty(s2))
            return true;
